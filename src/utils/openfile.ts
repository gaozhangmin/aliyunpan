...
import { IPageAudio, IPageBook, IPageCode, IPageImage, IPageOffice, IPageVideo } from '../store/appstore'
...
export async function menuOpenFile(file: IAliGetFileModel): Promise<void> {
  ...
  if (file.ext == 'djvu'
    ...
    // Book(drive_id, file_id, file.name, codeExt, file.size)
  ...
  if (file.category.startsWith('video')  && ';.ape.aac.cda.dsf.eac3.m1a.m2a.m4a.mka.mpa.mpc.opus.ra.tak.mp3.tta.wma.wv.'.indexOf(file.ext) <= 0) {
    ...
    if (uiVideoPlayer === 'other') {
      if (uiVideoSubtitleMode === 'auto') {
        ...
        if (drive_id === useUserStore().GetUserToken.backup_drive_id) {
          modalSelectPanDir(
              'backupPan',
              'select',
              parent_file_id,
              async (_user_id: string, _drive_id: string,  dirID: string, dirName: string) => {
                const subTitleFile =  {drive_id: _drive_id, file_id: dirID, name: dirName}
                await Video(token, file, subTitleFile)
              },
              '',
              /srt|vtt|ass/
          )
  ...
  if (file.category.startsWith('audio') || ';.ape.aac.cda.dsf.dtshd.eac3.m1a.m2a.m4a.mka.mpa.mpc.opus.ra.tak.mp3.tta.wma.wv.'.indexOf(file.ext) > 0) {
    Music(drive_id, file_id, file.name)
    return
  }
...
async function Music(drive_id: string, file_id: string, name: string): Promise<void> {
  const user_id = useUserStore().user_id
  const token = await UserDAL.GetUserTokenFromDB(user_id)
  if (!token || !token.access_token) {
    message.error('在线预览失败 账号失效，操作取消')
  ...
  if (drive_id === token.backup_drive_id) {
    ...
    fileList = useResPanFileStore().ListDataRaw
  }
  for (let i = 0, maxi = fileList.length; i < maxi; i++) {
    if (fileList[i].ext === 'ts') continue
    if (fileList[i].category.startsWith('audio') || ';.ape.aac.cda.dsf.dtshd.eac3.m1a.m2a.m4a.mka.mpa.mpc.opus.ra.tak.mp3.tta.wma.wv.'.indexOf(fileList[i].ext) > 0) {
      fileIdList.push(fileList[i].file_id)
  ...
  window.WebOpenWindow({ page: 'PageAudio', data: pageAudio, theme: 'dark', width:900, height:700 })
}

async function Video(token: ITokenInfo, file: IAliGetFileModel, subTitleFile: any): Promise<void> {
  ...
  if (uiAutoColorVideo && (!desc || !desc.includes('ce74c3c'))) {
    AliFileCmd.ApiFileColorBatch(token.user_id, file.drive_id, 'ce74c3c', [file.file_id])
        .then((success) => {
          if (file.drive_id == usePanFileStore().DriveID) {
            usePanFileStore().mColorFiles('ce74c3c', success)
          } else {
            useResPanFileStore().mColorFiles('ce74c3c', success)
          }
        })
  }
  ...
  if (uiVideoQualityTips || !uiVideoEnablePlayerList) {
    rawData = await getRawUrl(token.user_id, file.drive_id, file.file_id,file.icon == 'iconweifa', 'video')
...
async function Audio(drive_id: string, file_id: string, name: string, weifa: boolean): Promise<void> {
  ...
  const data = await getRawUrl(user_id, drive_id, file_id, weifa, 'audio')
...
async function Book(drive_id: string, file_id: string, name: string, codeExt: string, fileSize: number): Promise<void> {
  const user_id = useUserStore().user_id
  const token = await UserDAL.GetUserTokenFromDB(user_id)
  if (!token || !token.access_token) {
    message.error('在线预览失败 账号失效，操作取消')
  ...
  window.WebOpenWindow({ page: 'PageBook', data: pageBook, theme: 'dark' })
}
