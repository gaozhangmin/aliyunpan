...
import useResPanTreeStore, { fileiconfn } from './pantreestore'
...
import DirLeftMenu from './menus/ResDirLeftMenu.vue'
import { TreeNodeData } from '../store/treestore'
import { dropMoveSelectedFile } from './topbtns/ResTopbtn'
...
const handleTreeRightClick = (e: { event: MouseEvent; node: any }) => {
  ...
  onShowRightMenu('leftrespanmenu', e.event.clientX, e.event.clientY)
}
...
const onRowItemDrop = (ev: any, movetodirid: string) => {
  ...
  if (filesList && filesList.length > 0) {
    ...
    modalUpload('resourcePan', movetodirid, files)
  } else {
...
<template>
  <div style="width: 100%; height: 100%; overflow: hidden; min-width: 300px" tabindex="-1" @keydown.tab.prevent="() => true">
    <div class="headswitch">
      <div class="bghr"></div>
      <div class="sw">
        <MySwitchTab :name="'panleft'" :tabs="switchValues" :value="appStore.GetAppTabMenu" @update:value="(val:string)=>appStore.toggleTabMenu('resPan', val)" />
      </div>
    </div>
    <div class="treeleft">
      <a-tabs type="text" :direction="'horizontal'" class="hidetabs" :justify="true" :active-key="appStore.GetAppTabMenu">
        ...
        <a-tab-pane key="resKuaijie" title="2">
          <AntdTree
            :tabindex="-1"
            :focusable="false"
            class="colortree"
