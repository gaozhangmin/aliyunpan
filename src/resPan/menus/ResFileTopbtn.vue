...
import { menuDownload, menuFavSelectFile, menuTrashSelectFile, menuCopySelectedFile, menuJumpToDir, menuCreatShare, menuVideoXBT, menuDLNA, menuM3U8Download, menuCopyFileName, menuCopyFileTree } from '../topbtns/ResTopbtn'
...
<template>
  <div v-show="isselected && dirtype !== 'trash' && dirtype !== 'recover'" class="toppanbtn">
    ...
    <a-button v-show="dirtype == 'pan'" type="text" size="small" tabindex="-1" title="Ctrl+S" @click="() => menuCreatShare(istree, 'share')"><i class="iconfont iconfenxiang" />分享</a-button>
    <a-button v-show="dirtype == 'pan'" type="text" size="small" tabindex="-1" title="Ctrl+S" @click="() => menuCreatShare(istree, 'rapid')"><i class="iconfont iconfenxiang" />快传</a-button>
    ...
    <a-dropdown trigger="hover" class="rightmenu" position="bl">
      ...
      <template #content>
        <a-doption title="F2 / Ctrl+E" @click="() => modalRename('resourcePan', istree, isselectedmulti)">
        ...
        <a-doption title="Ctrl+P" @click="() => modalShuXing('resourcePan', istree, isselectedmulti)">
