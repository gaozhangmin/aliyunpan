...
const handlePlay = async (index: number) => {
  ...
  const user_id = useResPanTreeStore().user_id
  ...
  let rawData: any = await getRawUrl(user_id, first.drive_id, first.file_id, first.icon == 'iconweifa', 'video')
  ...
  modalSelectVideoQuality(info, rawData, async (quality: any) => {
    ...
    let subTitleUrl = ''
    const subTitlesList: IAliGetFileModel[] = useResPanFileStore().ListDataRaw.filter((file) => /srt|vtt|ass/.test(file.ext))
    const subTitleFile = PlayerUtils.filterSubtitleFile(first.name, subTitlesList)
    // 加载网盘内字幕文件
    if (subTitleFile) {
      const data = await getRawUrl(user_id, subTitleFile.drive_id, subTitleFile.file_id)
