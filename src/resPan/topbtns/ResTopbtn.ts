...
import BackupPanDAL from '../../pan/pandal'
...
export async function menuTrashSelectFile(istree: boolean, isDelete: boolean) {
  ...
  try {
    ...
    for (const file_id of selectedData.selectedKeys) {
      await DB.deleteValueString(file_id)
      await DB.deleteValueString(file_id+'movieinfo')
      await DB.deleteValueString(file_id+'season_poster')
      await DB.deleteValueString(file_id+'episode_name')
    }
    if (isDelete) {
    ...
    if (istree) {
      ...
      await DB.batchDeleteVideo(successList)
  ...
  if (copyToBackup) {
    modalSelectPanDir('backupPan', copyby, parent_file_id, async function (user_id: string, drive_id: string, dirID: string) {
      ...
      if (copyby == 'copy') {
        ...
        await BackupPanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, drive_id, dirID)
        await BackupPanDAL.aReLoadOneDirToShow(drive_id, dirID, true)
        ...
        await BackupPanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, selectedData.drive_id, parent_file_id)
        if (selectedData.drive_id != drive_id) {
          await BackupPanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, drive_id, dirID)
        }
        await BackupPanDAL.aReLoadOneDirToShow(drive_id, dirID, true)
    ...
    modalSelectPanDir('resourcePan', copyby, parent_file_id, async function (user_id: string, drive_id: string, dirID: string) {
      ...
      if (copyby == 'copy') {
        successList = await AliFileCmd.ApiCopyBatch(user_id, selectedData.drive_id, file_idList, drive_id, dirID)
        await PanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, drive_id, dirID)
        await PanDAL.aReLoadOneDirToShow(drive_id, dirID, true)
        TreeStore.ClearDirSize(drive_id, [dirID])
      } else {
        successList = await AliFileCmd.ApiMoveBatch(user_id, selectedData.drive_id, file_idList, drive_id, dirID)
        if (!istree) {
...
export function dropMoveSelectedFile(movetodirid: string) {
  ...
  AliFileCmd.ApiMoveBatch(selectedData.user_id, selectedData.drive_id, file_idList, selectedData.drive_id, movetodirid)
    .then(async (success: string[]) => {
      useResPanFileStore().mDeleteFiles(selectedData.dirID, success, true)
      await PanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, selectedData.drive_id, selectedData.dirID)
      await PanDAL.aReLoadOneDirToShow(selectedData.drive_id, movetodirid, true)
