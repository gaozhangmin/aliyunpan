...
import { useFootStore, useResPanFileStore, useResPanTreeStore } from '../../store'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleOpen = async () => {
      ...
      if (!file_id) {
        ...
        fileInfo.value = await AliFile.ApiFileInfoOpenApi(pantreeStore.user_id, pantreeStore.drive_id, file_id)
        ...
        if (fileInfo.value?.category == 'audio') {
          ...
          const video = await AliFile.ApiVideoPreviewUrlOpenApi(pantreeStore.user_id, pantreeStore.drive_id, file_id)
  ...
  methods: {
    ...
    handleCopyDownload() {
      const pantreeStore = useResPanTreeStore()
      AliFile.ApiFileDownloadUrlOpenApi(pantreeStore.user_id, pantreeStore.drive_id, this.fileInfo?.file_id || '', 14400).then((data) => {
