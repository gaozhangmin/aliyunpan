...
import { useResPanFileStore, useResPanTreeStore } from '../../store'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleMulti = () => {
      modalRename('resourcePan', props.istree, true)
    }
        ...
        AliFileCmd.ApiRenameBatch(pantreeStore.user_id, pantreeStore.drive_id, [this.form.file_id], [newName])
          .then((data) => {
            if (data.length == 1) {
              ...
              useResPanTreeStore().mRenameFiles(data)
              
              if (!this.istree) useResPanFileStore().mRenameFiles(data)
