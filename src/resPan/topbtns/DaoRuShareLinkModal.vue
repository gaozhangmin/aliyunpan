  ...
  methods: {
    ...
    handleOK() {
      this.formRef.validate((data: any) => {
        ...
        this.okLoading = true
        const urlSplitor = this.form.sharelink.split(/\.com\/s\/([\w]+)/)
        const share_id = urlSplitor[1]
        let dirId = ''
        if (urlSplitor.length == 3 && urlSplitor[2].includes('folder')) {
          dirId = urlSplitor[2].split('folder/')[1]
        }
        AliShare.ApiGetShareToken(share_id, this.form.password)
          .then((share_token) => {
            ...
            if (!share_token || share_token.startsWith('，')) {
              ...
              modalShowShareLink(share_id, this.form.password, share_token, true, [], dirId)
            }
          })
          .catch((err: any) => {
            this.okLoading = false
