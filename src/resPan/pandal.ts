...
import { useFootStore, useResPanFileStore } from '../store'
...
export default class PanDAL {
  ...
  static aTreeScrollToDir(dirID: string) {
    useResPanTreeStore().mSaveTreeScrollTo(dirID)
  }
  ...
  static GetDirFileList(user_id: string, drive_id: string, dirID: string, dirName: string, hasFiles: boolean = true): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      if (dirID == 'search') {
        if (hasFiles) useResPanFileStore().mSaveDirFileLoadingFinish(drive_id, dirID, [])
        resolve(true)
        return
      }
        ...
        .then((dir) => {
          if (!dir.next_marker) {
            TreeStore.SaveOneDirFileList(dir, hasFiles).then(() => {
              if (hasFiles) useResPanFileStore().mSaveDirFileLoadingFinish(drive_id, dirID, dir.items, dir.itemsTotal || 0)
              PanDAL.RefreshPanTreeAllNode(drive_id)
              resolve(true)
            })
  ...
  static GetPanSelectedData(istree: boolean): PanSelectedData {
    ...
    const data: PanSelectedData = {
      ...
      selectedKeys: istree ? [pantreeStore.selectDir.file_id] : useResPanFileStore().GetSelectedID(),
      selectedParentKeys: istree ? [pantreeStore.selectDir.parent_file_id] : useResPanFileStore().GetSelectedParentDirID()
    }
  ...
  static aReLoadQuickFile(user_id: string) {
    ...
    const arr = jsonstr ? JSON.parse(jsonstr) : []
    useResPanTreeStore().mSaveQuick(arr)
  }
