...
<template>
  <div v-show='!siteUrl'>
    <p class='site-title'>
      小白羊资源站点均来自互联网公开渠道,仅供学习交流使用,请勿用于商业用途。如有侵权,请发送邮件至zhangao456@proton.me,我们将立即删除相关内容。<br>
      小白羊不对资源的合法性、完整性、真实性负责,亦不承担由此引起的任何法律责任。
    </p>
    <a-tabs class='share-site-tabs'>
      ...
      <a-tab-pane title='全部'>
        <a-card :bordered='false' class='site-list'>
          <a-card-grid v-for='(siteItem, index) in serverStore.movieSiteList'
                       :key='index'
                       :hoverable='index % 2 === 0'
                       class='sitelistitem'>
