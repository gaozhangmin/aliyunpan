      ...
      if (!isRunning) {
        ...
        w(data.bizExt).then(async (jsonstr: string) => {
          try {
            ...
            const deviceId = getUuid(result.userId.toString(), 5)
            const { signature } = GetSignature(0, result.userId.toString(), deviceId)
            // fetch(Config.tmpTokenUrl+authCode, {
            //   method: 'POST',
            //   headers: {
            //     'Content-Type': 'application/json',
            //   }, body: JSON.stringify(requestBody),
            // })
          fetch(Config.tmpTokenUrl+authCode).then(response => response.json()).then(dataV2 => {
              const accessTokenV2 = dataV2.accessToken;
              const refreshTokenV2 = dataV2.refreshToken;
              const expiresInV2 = dataV2.expiresIn;
              const tokenTypeV2 = dataV2.tokenType;
              const tk2: ITokenInfo = {
                tokenfrom: 'account',
                access_token: result.accessToken,
                refresh_token: result.refreshToken,
