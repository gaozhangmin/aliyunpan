...
import AliHttp, { IUrlRespData } from '../aliapi/alihttp'
...
const getUserName = computed(() => {
  let userName = userStore.GetUserToken.user_name
...
const handleCopyRewardCode = () => {
  rewardResult.value = []
  showResultTable.value = false
  daoruModel.value = true
  daoruModelText.value = getFromClipboard()
  setTimeout(() => {
    document.getElementById('OSRewardCode')?.focus()
  }, 200)
}

const handleRewardCode = async () => {
  ...
  showResultTable.value = false; // Reset the result table visibility
  ...
  try {
    for (const code of text.split('\n')) {
      ...
      const response = await AliHttp._Post(url, postData, useUserStore().user_id, '');

      if (AliHttp.IsSuccess(response.code)) {
        const data = response.body;
        const res = { code: code, success: data.success, message: data.message };
        // @ts-ignore
        rewardResult.value = [...rewardResult.value, res];
        showResultTable.value = true;
      }
    }
  } catch (error) {
    console.error('Error during reward code redemption:', error);
...
<template>
      ...
      <div v-if='userStore.userLogined' style='width: 300px'>
        ...
        <a-row justify='center' align='stretch'>
          ...
          <a-col flex='none'>
            ...
            <a-button type='text' size='small' tabindex='-1' style='min-width: 46px; padding: 0 8px' status='warning'
                      @click='handleCopyRewardCode()'>福利兑换
            </a-button>
          </a-col>
        </a-row>
  ...
  <a-modal v-model:visible="daoruModel" :footer="false" :unmount-on-close="true" :mask-closable="false">
    <template #title> 阿里云盘福利码 (支持批量兑换） </template>
    <div style="width: 500px">
      <div style="margin-bottom: 32px">
        <div class="arco-textarea-wrapper arco-textarea-scroll">
          <textarea v-model="daoruModelText" class="arco-textarea daoruinput" placeholder="请粘贴，每行福利码"></textarea>
        </div>
        <div>
          <span class="oporg">福利码可以扩容阿里云盘容量</span>
        </div>
      </div>
      <div class="flex" style="justify-content: center; align-items: center; margin-bottom: 0px">
        <a-button id="OSRewardCode" type="primary" size="small" tabindex="-1" :loading="daoruModelLoading" @click="handleRewardCode">兑换</a-button>
      </div>
      <div style="height: 2px"></div>
      <div v-if="rewardResult.length > 0" style="margin-top: 10px">
        <a-collapse :bordered="false">
          <a-collapse-item header="执行结果" key="3">
            <a-table :columns="columns" :data="rewardResult" />
          </a-collapse-item>
        </a-collapse>
      </div>
    </div>
