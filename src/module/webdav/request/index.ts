...
import { usePanTreeStore, useSettingStore, useUserStore } from '../../../store'
...
class Request {
  ...
  static async getRootDirectory(ctx: IContextInfo): Promise<StructDirectory[]> {
    let backup_drive_id = GetDriveID(useUserStore().user_id, 'backup')
    let resource_drive_id = GetDriveID(useUserStore().user_id, 'resource')
    let pic_drive_id = GetDriveID(useUserStore().user_id, 'pic')
  ...
  static async getStructDirectory(ctx: IContextInfo, drive_id: string, file_id: string): Promise<StructDirectory[] | StructDirectory | Error> {
    ...
    try {
      ...
      let files: any[] = []
      let folders: any[] = []
      const resp = await AliDirFileList.ApiDirFileList(user_id, drive_id, file_id, '', 'name asc', '',  true, false)
  ...
  static async getReadStream(ctx: OpenReadStreamInfo, file: any) {
    try {
      ...
      if (!this.fileInfo || this.fileInfo.file_id != file.file_id) {
        const data = await getRawUrl(usePanTreeStore().user_id, file.drive_id, file.file_id)
        let url = ''
