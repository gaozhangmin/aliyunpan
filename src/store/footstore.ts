...
const useFootStore = defineStore('foot', {
  ...
  getters: {
    ...
    GetSpaceInfo(state: FootState): string {
      ...
      if (appTab == 'pan' || appTab == 'resPan') return state.panSpaceInfo
    ...
    GetInfo(state: FootState): string {
      ...
      if (appTab == 'pan' || appTab == 'resPan') return state.panDirInfo
            ...
            .then(async (resp) => {
              ...
              if (item.status != 'running') {
                ...
                if (item.type == '解压' || item.type == '复制' || item.type == '导入分享' || item.type == '回收站还原') {
                  if (item.todrive_id === useUserStore().GetUserToken.backup_drive_id) {
                    await PanDAL.aReLoadOneDirToShow(item.todrive_id, 'refresh', false)
                    message.success(`${item.type}完成`)
                  } else {
                    await ResPanDAL.aReLoadOneDirToShow(item.todrive_id, 'refresh', false)
                    message.success(`${item.type}完成`)
                  }
