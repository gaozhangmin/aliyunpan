  ...
  actions: {
    ...
    async toggleTab(tab: string) {
      if (this.appTab != tab) {
        ...
        if (tab == 'movie' || tab == 'search') {
          const phone = useUserStore().GetUserToken.phone
          this.isVip = await DB.getValueBool(phone)
          if (useUserStore().userLogined && !this.isVip) {
            if (useUserStore().GetUserToken.phone === undefined) {
              message.warning("获取VIP数据失败，稍候重试")
            } else {
              this.isVip = await AliHttp.isVip(phone);
              DB.saveValueBool(phone, this.isVip)
            }
          }
        }
        onHideRightMenu()
    ...
    toggleTabNextMenu() {
      ...
      switch (this.appTab) {
        ...
        case 'share': {
          next(this.appTabMenuMap, this.appTab, ['ShareSiteRight', 'OtherShareRight', 'MyShareRight', 'MyTransferShareRight', 'MyFollowingRight', 'OtherFollowingRight', 'alist'])
          break
        }
        case 'rss': {
        ...
        case 'setting': {
          next(this.appTabMenuMap, this.appTab, ['SettingUI', 'SettingDown', 'SettingPan', 'SettingDebug', 'SettingAria', 'SettingWebDav', 'SettingLog'])
        ...
        case 'search': {
          next(this.appTabMenuMap, this.appTab, ['search', ''])
          break
        }
      }
