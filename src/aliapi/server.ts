    ...
    axios
      ...
      .then(async (response: AxiosResponse) => {
        ...
        if (response.data.movieList && response.data.movieList.length > 0) {
          const list: IShareSiteModel[] = []
          for (let i = 0, maxi = response.data.movieList.length; i < maxi; i++) {
            const item = response.data.movieList[i]
            const add = { title: item.title, url: item.url, tip: item.tip, group: 'video', color: '', external: ''}
            if (add.url.length > 0) list.push(add)
          }
          ShareDAL.SaveMovieSite(list)
        }
        if (response.data.HELP && response.data.HELP.length > 0) {
          useServerStore().mSaveHelpUrl(response.data.HELP)
        }
        if (response.data.salesImg) {
          useServerStore().mSaveSalesUrl((response.data.salesImg))
        }
        const isVip = await DB.getValueBool(useUserStore().GetUserToken.phone)
        if (isVip && response.data.tmdbProxyUrl) {
          useServerStore().mSaveProxyUrl((response.data.tmdbProxyUrl))
        }
      })
  }
