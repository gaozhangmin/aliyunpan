import { usePanFileStore, useResPanFileStore, useServerStore, useSettingStore } from '../store'
...
export default class AliDirFileList {
  ...
  static getFileInfo(item: IAliFileItem, downUrl: string): IAliGetFileModel {
    ...
    const { name, mine_type, ext } = DecodeName(item)
  ...
  static async ApiDirFileList(user_id: string, drive_id: string, dirID: string, dirName: string, order: string, type: string = '', openApi = false, search = true): Promise<IAliFileResp> {
  ...
  static async ApiDirFileSize(user_id: string, drive_id: string, file_idList: string[]): Promise<{ dirID: string; size: number }[] | undefined> {
    ...
    for (let i = 0, maxi = file_idList.length; i < maxi; i++) {
      ...
      list.set(id, { dirID: file_idList[i], size: 0 })
      const postData = {
        drive_id: drive_id,
