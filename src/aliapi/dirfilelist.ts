...
export default class AliDirFileList {
  ...
  static LimitMax = 100
  static ItemJsonmask = 'category%2Ccreated_at%2Cdomain_id%2Cdrive_id%2Cfile_extension%2Cfile_id%2Chidden%2Cmime_extension%2Cmime_type%2Cname%2Cparent_file_id%2Cpunish_flag%2Csize%2Cstarred%2Ctype%2Cupdated_at%2Cdescription%2Cthumbnail'
  ...
  static async ApiDirFileList(user_id: string, drive_id: string, dirID: string, dirName: string, order: string, type: string = '', openApi = false): Promise<IAliFileResp> {
    ...
    if (useSettingStore().uiFileListMode === 'movie') {
  ...
  static async searchTMDB(dir: IAliFileResp): Promise<void> {
    // const regex = /(?:s|S)(\d+)(?:e|E)(\d+)|(\d{4})|(\d{3,4}p)/g;
    // const regex = /(?:s|S)(\d+)(?:(?:e|E)(\d+))?|\d{4}|\d{3,4}p/g;
    // const regex = /(?:s|S)(\d+)(?:(?:e|E)(\d+))?|(\d{4})|(\d{3,4}p)|([sS][eE]\d+)/g
    ...
    for (const fileItem of dir.items) {
      ...
      //   continue
      // }
      if (fileItem.hasOwnProperty('minfo') && fileItem.minfo !== undefined) {
        continue
      }
      ...
      if (tmdb_cache) {
        ...
        if (media_type === 'tv') {
          const urlTv = `https://movie-api.infuse.im/3/tv/${tmdb_id}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
          ...
          const url = `https://movie-api.infuse.im/3/movie/${tmdb_id}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
      ...
      if (!fileItem.minfo) {
        ...
        const searchName = parts[0].replaceAll('.', ' ').replace(/\[.*?\]/g, '').replaceAll('&', ' ')
        if (searchName == '4k' || searchName == '4K' || searchName == '字幕' || searchName == 'test') continue
        const url = `https://api.themoviedb.org/3/search/multi?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN&query=${searchName}&page=1&include_adult=false`
        const resp = await AliHttp.GetWithOutUserId(url);
        if (AliHttp.IsSuccess(resp.code) && resp.body.results.length > 0) {
      ...
      if (fileItem.minfo && fileItem.minfo.media_type === 'tv') {
        ...
        if (Number(seasonNum) != -1) {
          if (Number(episodeNum) != -1) {
            const urlSeason = `https://movie-api.infuse.im/3/tv/${fileItem.minfo.id}/season/${fileItem.season_num}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
            ...
            if (AliHttp.IsSuccess(resp.code)) {
              ...
              // @ts-ignore
              fileItem.episode_name = resp.body.episodes.filter(episode => episode.episode_number === fileItem.episode_num)[0].name
            }
          } else {
            const tvApi = `https://api.themoviedb.org/3/tv/${fileItem.minfo.id}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
            const tvResp = await AliHttp.GetWithOutUserId(tvApi)
            if (AliHttp.IsSuccess(tvResp.code) && tvResp.body.seasons.length > 0) {
