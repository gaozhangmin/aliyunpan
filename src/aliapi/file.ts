...
import { GetExpiresTime, GetOssExpires, HanToPin } from '../utils/utils'
...
import {
  ...
  IDownloadUrl,
  IOfficePreViewUrl,
  IVideoPreviewUrl, IVideoPreviewUrlModal,
  IVideoXBTUrl
} from './models'
...
export default class AliFile {
  ...
  static async ApiVideoPreviewUrlOpenApi(user_id: string, drive_id: string, file_id: string): Promise<IVideoPreviewUrl | string> {
    if (!user_id || !drive_id || !file_id) return '参数错误'
    ...
    if (AliHttp.IsSuccess(resp.code)) {
      ...
      for (const item of items) {
        data.push({
          ...
          url: item.url,
          expire_sec: GetExpiresTime(item.url),
