    ...
    while (true) {
      ...
      try {
        if (AliHttp.IsSuccess(resp.code)) {
          ...
          DebugLog.mSaveWarning('SSApiBatchDirFileList1 err=' + (resp.code || ''))
        }
      } catch (err: any) {
  ...
  static async ApiFastAllDirListByPID(user_id: string, drive_id: string): Promise<IDirDataResp> {
    ...
    while (true) {
      ...
      for (let i = 0, maxi = dirList.length; i < maxi; i++) {
        const postData = {
          drive_id: drive_id,
          query: 'type="folder" and ' + dirList[i].dirID,
          marker: dirList[i].next_marker,
          limit: 100,
          // fields: 'thumbnail',
          order_by: 'name ASC'
        ...
        try {
          if (AliHttp.IsSuccess(resp.code)) {
            ...
            errorMessage = (resp.code || '').toString()
            DebugLog.mSaveWarning('SSApiBatchDirFileList123 err=' + (resp.body || ''))
          }
        } catch (err: any) {
          errorMessage = err.message || ''
          DebugLog.mSaveWarning('ApiBatchDirFileList', err)
