import { ITokenInfo, useFootStore, useSettingStore } from '../store'
...
async function _ApiUpdateFile(postData: any, user_id: string, share_token: string, result: IAliBatchResult): Promise<void> {
  if (!user_id && !share_token) return
  const url = 'adrive/v3/file/update'
  const respi = await AliHttp.Post(url, postData, user_id, share_token)
  if (AliHttp.IsSuccess(respi.code)) {
    const status = respi.code as number
    if (status >= 200 && status <= 205) {
      result.count++
      if (respi.body && respi.body.async_task_id) {


        result.async_task.push({ drive_id: respi.body.drive_id || '', file_id: respi.body.id, task_id: respi.body.async_task_id, newdrive_id: respi.body.drive_id || '', newfile_id: respi.body.file_id || '' })
      } else if (respi.body && respi.body.share_id && respi.body.share_msg) {

        result.reslut.push({ id: respi.body.file_id, share_id: respi.body.share_id, share_pwd: respi.body.share_pwd || '', share_url: respi.body.share_url, expiration: respi.body.expiration || '', share_name: respi.body.share_name || '' })
      } else if (respi.body) {
        result.reslut.push({ id: respi.body.file_id, file_id: respi.body.file_id, name: respi.body.name || '', body: respi.body })
      }
    } else {
      ...
      if (respi.body && respi.body.code) result.error.push({ id: respi.body.id || respi.body.file_id, code: respi.body.code, message: respi.body.message })
    }
  } else {
    DebugLog.mSaveWarning('_ApiUpdateFile err=' + (respi.code || ''))
  }
}
...
export async function ApiUpdateFileSuccess(title: string, batchList: any[], user_id: string, share_token: string): Promise<string[]> {
  if (batchList.length == 0) return Promise.resolve(batchList)
  const successList: string[] = []
  const result = await ApiBatch(title, batchList, user_id, share_token)
  result.reslut.map((t) => successList.push(t.id))
...
export function DecodeName(item: IAliFileItem | IAliGetFileModel) {
  // 自动解密文件名
  let ext = ''
