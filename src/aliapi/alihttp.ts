...
export default class AliHttp {
  ...
  private static _GetBlob(url: string, user_id: string): Promise<IUrlRespData> {
    return UserDAL.GetUserTokenFromDB(user_id).then((token) => {
      const headers: any = {}
      if (token) {
        let token_type = token.token_type || 'Bearer'
        let access_token = token.access_token
        let need_open_api = url.includes('openapi')
        if (need_open_api && token.open_api_access_token) {
          token_type = token.open_api_token_type || 'Bearer'
  ...
  static async isVip(phone:string): Promise<boolean> {
    ...
    if (AliHttp.IsSuccess(response.status)) {
      ...
      if (json.data.rows.length > 0) {
        const data = json.data.rows as {vip_type:string, update_time:string}[];
        if (data[0].vip_type === 'promotion') {
          return true
        } else if(data.length > 0 && data[0].vip_type !== '' &&  data[0].update_time !== '') {
          return !AliHttp.vipExpire(data[0].update_time, data[0].vip_type)
        } else {
          return false
        }
      } else {
  ...
  static vipExpire(update_time: string, vip_type: string): boolean {
    const now = new Date().getTime();
    const pay = new Date(update_time).getTime();
    const diff = now - pay;
    const releaseDate = new Date('2023-10-31').getTime();

    if (pay <= releaseDate) {
      return false;
    }
    if (vip_type === 'month_vip_8t') {
      // 月度VIP，规则为支付后30天过期
      if (diff > 31 * 24 * 60 * 60 * 1000) {
        return true;
      }
    } else if (vip_type === 'year_vip_8t') {
      // 年度VIP（8t），规则为支付后365天过期
      if (diff > 366 * 24 * 60 * 60 * 1000) {
        return true;
      }
    } else if (vip_type === 'year_vip') {
      // 年度VIP，规则为支付后365天过期
      if (diff > 366 * 24 * 60 * 60 * 1000) {
        return true;
      }
    } else if (vip_type === 'year') {
      // 年度普通会员，规则为支付后365天过期
      if (diff > 366 * 24 * 60 * 60 * 1000) {
        return true;
      }
