...
import { useDowningStore, useSettingStore, useUserStore } from '../../store'
  ...
  if (copyToRes) {
    modalSelectPanDir('resourcePan', copyby, parent_file_id, async function (user_id: string, drive_id: string, dirID: string) {
      ...
      if (copyby == 'copy') {
        ...
        await ResPanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, drive_id, dirID)
        await ResPanDAL.aReLoadOneDirToShow(drive_id, dirID, true)
        ...
        await ResPanDAL.aReLoadOneDirToRefreshTree(selectedData.user_id, selectedData.drive_id, parent_file_id)
