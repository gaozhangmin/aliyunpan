  ...
  setup(props) {
    ...
    const fetchMovie = async () => {
      if (movieMap.has(movieid.value)) {
        ...
        const url = `https://movie-api.infuse.im/3/movie/${movieid.value}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
    ...
    const fetchTV = async () => {
      if (tvSeriesMap.has(movieid.value)) {
        ...
        const urlTv = `https://movie-api.infuse.im/3/tv/${movieid.value}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
      ...
      if (tvSeasonsMap.has(movieid.value)) {
        const data = tvSeasonsMap.get(movieid.value)
        // @ts-ignore
        ...
        const urlSeason = `https://movie-api.infuse.im/3/tv/${movieid.value}/season/${tvSeasonNum.value}?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
        ...
        if (AliHttp.IsSuccess(resp.code)) {
          ...
          tvSeasonsMap.set(movieid.value, resp.body)
    ...
    const fetchTvCast = async () => {
      if (castMap.has(movieid.value+tvSeasonNum.value)) {
        ...
        const url = `https://movie-api.infuse.im/3/tv/${movieid.value}/season/${tvSeasonNum.value}/credits?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
        const resp = await AliHttp.GetWithOutUserId(url)
        if (AliHttp.IsSuccess(resp.code) ) {
    ...
    const fetchCast = async () => {
      if (castMap.has(movieid.value)) {
        ...
        const url = `https://movie-api.infuse.im/3/movie/${movieid.value}/credits?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN`
        const resp = await AliHttp.GetWithOutUserId(url)
        if (AliHttp.IsSuccess(resp.code) ) {
...
<!--      <span class="loader m-10"></span>-->
    ...
    <div class="scroll-container" v-else>
      ...
      <!-- poster -->
      <div class="relative h-auto md:h-[82vh] flex justify-center overflow-hidden">
        ...
        <div class="absolute bottom-5 left-0 p-3 justify-between  items-center space-y-2">
          ...
          <div class="absolute bottom-1 text-white text-left play-button overflow-scroll" style='margin-left: 350px'>
            ...
            <div class="flex items-center mt-2 mb-2 "> <!-- 使用 Flexbox 容器 -->
              ...
              <p v-if='tvepisodedet.vote_average > 0' class="text-sm font-semibold ml-2 text-blue-400">{{ tvepisodedet.vote_average.toFixed(1)  }}</p>
              <p v-else-if='moviedet.vote_average > 0' class="text-sm font-semibold ml-2 text-blue-400 ">{{ moviedet.vote_average.toFixed(1) }}</p>
              <p v-else  class="text-sm font-semibold ml-2 text-blue-300 ">{{ 0 }}</p>
              <p v-show='movieFile.media_duration || movieFile.duration' class="text-sm font-semibold ml-5 text-blue-400 ">时长: {{movieFile.media_duration || humanTime(movieFile.duration || 0) || '' }}</p>
              <p v-show='movieFile.media_height != undefined' class="text-sm font-semibold ml-5 text-blue-400 ">{{ movieFile.media_height?.toString()+'p'}}</p>
              <p v-if='tvepisodedet.air_date' class="text-sm font-semibold ml-5 text-blue-400 ">首映: {{ tvepisodedet.air_date }}</p>
              <p v-else-if='moviedet.release_date' class="text-sm font-semibold ml-5 text-blue-400 ">首映: {{ moviedet.release_date }}</p>
              <p v-show='moviedet.last_air_date' class="text-sm font-semibold ml-5 text-blue-400 ">更新: {{ moviedet.last_air_date }}</p>
              <p v-show='moviedet.number_of_seasons' class="text-sm font-semibold ml-5 text-blue-400 ">总{{ moviedet.number_of_seasons }}季-总{{ moviedet.number_of_episodes }}集</p>
              <p v-show='getCountry()' class="text-sm font-semibold ml-5 text-blue-400 ">地区: {{ getCountry() }}</p>
              <p v-show='translateStatus(moviedet.status)' class="text-sm font-semibold ml-5 text-blue-400 ">{{ translateStatus(moviedet.status) }}</p>
            </div>
      ...
      <!-- 电影列表 -->
      <div class="flex flex-col mt-10 movie-container ml-5">
        <div
          ...
          <img
            v-else
            :src="movie.thumbnail"
            class="w-40 h-full rounded-xl postimg"
          />
      ...
      <!-- cast -->
      <div v-if='moviedet.id !== -1 && castdata.length > 0' class="cast-container flex-col items-center">
