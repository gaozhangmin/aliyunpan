...
function timeEvent() {
  ...
  if (chkUpgradeTime1 > 0 && nowTime - chkUpgradeTime1 > 14400) {
    chkUpgradeTime1 = nowTime
    const phone = useUserStore().GetUserToken.phone
    AliHttp.isVip(phone).then((isVip)  => {
      DB.saveValueBool(phone, isVip)
    }).catch((err: any) => {
      DebugLog.mSaveDanger('isVip', err)
    })
