...
import { getFromClipboard, getResourcesPath, openExternal } from '../utils/electronhelper'
...
import Movie from '../resource/MovieArea.vue'
import Search from '../share/share/ShareSiteRight.vue'
...
keyboardStore.$subscribe((_m: any, state: KeyboardState) => {
  if (TestAlt('1', state.KeyDownEvent, () => appStore.toggleTab('resPan'))) return
  if (TestAlt('2', state.KeyDownEvent, () => appStore.toggleTab('pan'))) return
...
onMounted(() => {
  onResize()
  theme.value = settingStore.uiTheme
...
const cb = (val: any) => {
  theme.value = val.uiTheme
  settingStore.updateStore(val)
}

const themeIcon = computed(() => {
  return theme.value === 'dark' ? 'iconfont iconday' : 'iconfont iconnight';
});
  ...
  <a-layout style="height: 100vh" draggable="false">
    <a-layout-header id="xbyhead" draggable="false">
      <div id="xbyhead2" class="q-electron-drag">
        <a-button v-show="appStore.appTab === 'pan' || appStore.appTab === 'resPan'" type="text" size="small" @click="handlePanVisible">
        ...
        <a-menu class="custom-menu" mode="horizontal" :selected-keys="[appStore.appTab]" @update:selected-keys="appStore.toggleTab($event[0])">
          <a-menu-item key="resPan" title="Alt+1">资源盘</a-menu-item>
          <a-menu-item key="pan" title="Alt+2">备份盘</a-menu-item>
          <a-menu-item key="pic" title="Alt+3">相册</a-menu-item>
          <a-menu-item key="down" title="Alt+4">传输</a-menu-item>
          <a-menu-item key="share" title="Alt+5">资源分享</a-menu-item>
          <a-menu-item key="rss" title="Alt+6">插件</a-menu-item>
<!--          <a-sub-menu key="movie" title="Alt+7" class="custom-sub-menu">-->
<!--            <template #title><i class='iconfont iconcrown' style='color: red; font-size: 15px;'/>4K影视</template>-->
<!--            <a-menu-item key="xby">小白羊影视</a-menu-item>-->
<!--            <a-menu-item key="4k">4K电影</a-menu-item>-->
<!--          </a-sub-menu>-->
          <a-menu-item key="movie" title="Alt+7"><i class='iconfont iconcrown' style='color: red; font-size: 15px;'/>4K影视专区</a-menu-item>
          <a-menu-item key="search" title="Alt+8"><i class='iconfont iconcrown' style='color: red; font-size: 15px;'/>资源专区</a-menu-item>
        </a-menu>
        ...
        <UserInfo />
        <UserLogin />
        <a-popover trigger='hover' position='top' class='asynclist'>
            ...
            <div style="display:flex;">
              <div style="flex: 1;">
                <AntQRCode
                    ...
                    bg-color="#F0F8FF"
                    :size="250" />
                <p class="image-text2 ml-14">阿里云官方购买会员返佣码</p>
              </div>
              <div style="flex: 1;">
                <img style="width: 250px;height: 250px" src='/images/qrcode_258.jpg' alt='公众号'>
                <p class="image-text2 ml-14">公众号二维码</p>
              </div>
            </div>
          </template>
        </a-popover>
        <a-button type="text" tabindex="-1"  title="主题切换"  @click="cb({ uiTheme:  theme == 'dark' ? 'light' : 'dark'})">
          <i :class="themeIcon"></i>
        </a-button>
    ...
    <a-layout-content id="xbybody">
      <a-tabs type="text" :direction="'horizontal'" class="hidetabs" :justify="true" :active-key="appStore.appTab">
        <a-tab-pane key="resPan" title="1"><ResourcePan :visible="panVisible"/></a-tab-pane>
        <a-tab-pane key="pan" title="2"><Pan :visible="panVisible"/></a-tab-pane>
        ...
        <a-tab-pane v-if='appStore.isVip' key="search" title="8"><Search /></a-tab-pane>
        <a-tab-pane v-if='!appStore.isVip' key="search" title="8"><VipInformPage /></a-tab-pane>
      </a-tabs>
    </a-layout-content>
