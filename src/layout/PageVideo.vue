...
import { KeyboardState, useAppStore, useKeyboardStore, usePanFileStore, useResPanFileStore, useSettingStore } from '../store'
...
import { getRawUrl, IRawUrl } from '../utils/proxyhelper'
...
const autoPlayPrev = async () => {
  ...
  if (item.file_id !== pageVideo.file_id) {
    ...
    await refreshPlayList(ArtPlayerRef, item.file_id)
  } else {
    autoPlayPrev()
  }
}
...
const getDirFileList = async (dir_id: string, hasDir: boolean, category: string = '', filter?: RegExp): Promise<any[]> => {
  if (curDirFileList.length === 0 || (hasDir && childDirFileList.length === 0)) {
    const dir = await AliDirFileList.ApiDirFileList(pageVideo.user_id, pageVideo.drive_id, dir_id, '', 'name asc', '', true, false)
...
const getVideoInfo = async (art: Artplayer) => {
  // 获取视频链接
  const data: string | IRawUrl = await getRawUrl(pageVideo.user_id, pageVideo.drive_id, pageVideo.file_id, false, 'video')
