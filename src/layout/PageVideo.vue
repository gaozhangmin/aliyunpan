...
import { useAppStore, usePanFileStore, useResPanFileStore, useSettingStore, useUserStore } from '../store'
...
const options: Option = {
  ...
  icons: {
    ...
    state: '<img width="150" heigth="150" src="./images/state.svg">',
    indicator: '<img width="16" heigth="16" src="./images/indicator.svg">',
    // play: '<svg  style="position: absolute ;left:-10px ; top: 9px;  width: 30px; height: 30px" ><path d="M0 .984v14.032a1 1 0 0 0 1.506.845l12.006-7.016a.974.974 0 0 0 0-1.69L1.506.139A1 1 0 0 0 0 .984Z"/></svg>',
    // pause: '<img style="position: absolute ;left:-10px; width: 30px; height: 30px"  src="./images/pause.svg">',
  },
  controls: [
    {
      name: 'arrowLeft',
...
const getDirFileList = async (dir_id: string, hasDir: boolean, category: string = '', filter?: RegExp): Promise<any[]> => {
  if (curDirFileList.length === 0 || (hasDir && childDirFileList.length === 0)) {
    ...
    if (!dir.next_marker || dir.next_marker == 'cancel') {
  ...
  art.setting.add(
      {
        ...
        onChange(item: SettingOption) {
          art.storage.set('autoSkipBegin', item.range*60)
...
const getVideoInfo = async (art: Artplayer) => {
  ...
  const originData = await AliFile.ApiFileDownloadUrlOpenApi(pageVideo.user_id, pageVideo.drive_id, pageVideo.file_id)
  ...
  if (data) {
    ...
    if (data.urlQHD) qualitySelector.push({ url: data.urlQHD, html: '4K' })
    ...
    let qualityDefault
    if (lastPlayedQuality != '') {
      qualityDefault = qualitySelector.find((item) => item.html === lastPlayedQuality)
      if (qualityDefault) {
        qualityDefault.default = true
      } else {
        qualityDefault = qualitySelector.find((item) => item.default) || qualitySelector[0]
        qualityDefault.default = true
      }
    } else {
      qualityDefault = qualitySelector.find((item) => item.default) || qualitySelector[0]
...
const updateVideoTime =  async () => {
  const watched = ArtPlayerRef.currentTime >= ArtPlayerRef.duration - 60
  await DB.saveVideoInfo(pageVideo.file_id,
    {file_id: pageVideo.file_id, drive_id: pageVideo.drive_id,
      play_cursor: ArtPlayerRef.currentTime, duration: ArtPlayerRef.duration, watched: watched}
  )
