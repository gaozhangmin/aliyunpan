...
export default defineComponent({
  ...
  setup(props) {
    ...
    const searchTMDB = async () => {
      const url = `${useServerStore().proxyUrl}/3/search/multi?api_key=87d8eb3d0895eaf37c2929fd5d9f7cce&language=zh-CN&query=${form.name.trim()}&page=1&include_adult=false`
    ...
    const selectMeta = async() => {
      ...
      if (selectedMovie.value.id) {
        ...
        await DB.deleteValueString(props.movies[0].file_id + "movieinfo")
        await DB.deleteValueString(props.movies[0].file_id + "season_poster")
        await DB.deleteValueString(props.movies[0].file_id + "episode_name")
      }
      modalCloseAll()
    }
