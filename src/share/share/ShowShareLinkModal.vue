...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleOpen = () => {
      ...
      if (props.share_token) {
        ...
        if (props.dirId != '') {
          apiLoad(props.dirId).then((addList: TreeNodeData[]) => {
    ...
    const handleFilterChange = (event) => {
      filterKeyword.value = event.target.value
    }
  ...
  methods: {
    ...
    async handleOK(saveType: string, panType:string) {
      ...
      const treeData = this.filteredTreeData
      ...
      modalSelectPanDir(panType, 'share', '', async function(user_id: string, drive_id: string, dirID: string) {
...
<template>
  <a-modal :visible='visible' modal-class='modalclass showsharemodal' title-align='start' :footer='false'
    ...
    <template #title>
      ...
      <input v-model="filterKeyword" @input="handleFilterChange" placeholder="输入关键词筛选文件">
    </template>
    ...
    <div v-if='withsave && isAlbum == false' class='modalfoot'>
      ...
      <a-button type='primary' status='success' size='small' tabindex='-1' :loading='okLoading'
                title='按照分享链接内部的路径，导入勾选的文件和文件夹' @click="() => handleOK('folder',  panType)">导入勾选(按路径)
      </a-button>
      <a-button type='primary' status='normal' size='small' tabindex='-1' :loading='okLoading'
                title='导入勾选的文件(包含选中的文件夹)' @click="() => handleOK('fileAndFolder',  panType)">导入勾选(仅选中文件及文件夹)
      </a-button>
      <a-button type='primary' status='warning' size='small' tabindex='-1' :loading='okLoading'
                title='导入勾选的文件(把所有文件都保存到同一个文件夹里)' @click="() => handleOK('file',  panType)">导入勾选(仅文件)
      </a-button>
      <a-button type='primary' status='danger' size='small' tabindex='-1' :loading='okLoading'
                title='一键导入分享链接内全部文件和文件夹' @click="() => handleOK('all',  panType)">一键导入全部
      </a-button>
    </div>
