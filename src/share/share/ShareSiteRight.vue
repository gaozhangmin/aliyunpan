...
const handleSite = (item: IShareSiteModel) => {
  if (item.url.startsWith('http')) {
    ...
    if (item.url) siteUrl.value = item.url
    else siteUrl.value = ''
  }
...
const handleSiteShareUrl = (event: any) => {
  // 获取点击的 URL
  const url = event.url || ''
  window.WebToElectron({ cmd: 'closeFocus' })
  ...
  <div class='toppanbtns' style='height: 36px' v-show='siteUrl'>
    ...
    <div class='toppanbtn'>
      <a-button type='text' size='small' tabindex='-1' @click='handleBack'>
        <i class='iconfont iconarrow-left-1-icon' style='color: #0abb87' />回退
      </a-button>
    </div>
    <div class='toppanbtn'>
      <a-button type='text' size='small' tabindex='-1' @click='handleForward'>
        <i class='iconfont iconarrow-right-1-icon' style='color: #0abb87'/>前进
      </a-button>
    </div>
  </div>
