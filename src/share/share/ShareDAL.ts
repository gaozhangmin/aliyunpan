...
export default class ShareDAL {
  ...
  static async aLoadFromDB(): Promise<void> {
    ...
    const movieSiteList = await DB.getValueObject('movieSiteList')
    useServerStore().mSaveMovieSiteList(movieSiteList as IShareSiteModel[])
  ...
  static async aReloadHistoryShare(user_id: string, force: boolean): Promise<void> {
    if (!user_id) return
    const myshareStore = useHistoryShareStore()
    if (!force && myshareStore.ListDataRaw.length > 0) return
    if (myshareStore.ListLoading == true) return
    myshareStore.ListLoading = true
    const resp = await AliShareList.ApiHistoryShareListAll(user_id)
    myshareStore.aLoadListData(resp.items)
    myshareStore.ListLoading = false
  }
  ...
  static SaveMovieSite(list: IShareSiteModel[]) {
    DB.saveValueObject('movieSiteList', list).catch(() => {})
    useServerStore().mSaveMovieSiteList(list)
  }
}
