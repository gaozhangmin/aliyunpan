...
const RefreshTree = (checkall: boolean): void => {
  ...
  const treeDataNodes: TreeNodeData[] = GetTreeNodes(ScanPanData, 'root', treeDataMap, ShowWeiGui.value, ShowPartWeiGui.value, ShowNoShare.value)
...
const handleMove = () => {
  ...
  modalSelectPanDir(panType.value+'pan','cut', '', function (user_id: string, drive_id: string, dirID: string) {
    if (!drive_id || !dirID) return 
    delLoading.value = true
    AliFileCmd.ApiMoveBatch(user.user_id, drive_id, checkedKeys.value, drive_id, dirID).then((success: string[]) => {
      delLoading.value = false
      if (checkedKeys.value.length == checkedKeysBak.length) {
        handleReset()
      } else {
