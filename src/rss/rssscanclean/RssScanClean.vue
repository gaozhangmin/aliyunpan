...
const handleDelete = () => {
  ...
  let driveId = ''
  if (scanDrive.value === 'backup') {
    driveId = user.backup_drive_id
  } else {
    driveId = user.resource_drive_id
  }
  AliFileCmd.ApiTrashBatch(user.user_id, driveId, checkedKeys.value).then((success: string[]) => {
    delLoading.value = false
...
const handleScan = () => {
  ...
  LoadScanDir(user.user_id, driveId, totalDirCount, Processing, ScanPanData)
    .then(() => {
  ...
  <div class="scanfill rightbg">
    ...
    <div class="settingcard scanauto" style="padding: 4px; margin-top: 4px">
      <a-row justify="space-between" align="center" style="margin: 12px; height: 28px; flex-grow: 0; flex-shrink: 0; flex-wrap: nowrap; overflow: hidden">
        ...
        <div v-else>
          <a-select v-model:model-value="scanDrive" size="small" tabindex="-1" :style="{ width: '136px' }" :disabled="scanLoading" style="margin-right: 12px">
            <a-option value="backup">备份盘</a-option>
            <a-option value="resource">资源盘</a-option>
          </a-select>
          <a-select v-model:model-value="scanType" size="small" tabindex="-1" style="width: 136px; flex-shrink: 0; margin-right: 12px" :disabled="scanLoading">
